...
public class Velocity extends Module {
	...
	public final BooleanSetting inBlock = add(new BooleanSetting("InBlock", true)); // 新增选项
	...
	@Override
	public void onUpdate() { // 新增 onUpdate 方法
		if (nullCheck() || !inBlock.getValue()) return;

		BlockPos playerPos = mc.player.getBlockPos();
		if (mc.world.getBlockState(playerPos).getBlock() == Blocks.OBSIDIAN) {
			// 强制减速（设置为正常速度的 50%）
			mc.player.setVelocity(
					mc.player.getVelocity().x * 0.5,
					mc.player.getVelocity().y,
					mc.player.getVelocity().z * 0.5
			);
		}
	}
